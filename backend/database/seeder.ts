...
class DatabaseSeeder {
  ...
  async init() {
    ...
    this.productColorSizeRepository =
      this.connection.getRepository(ProductColorSize);
  ...
  private async seedProductColorSizes() {
    for (const pcs of productColorSizes) {
      const product = await this.productRepository.findOne({
        where: { id: pcs.product_id },
      });
      const color = await this.colorRepository.findOne({
        where: { id: pcs.color_id },
      });
      const size = await this.sizeRepository.findOne({
        where: { id: pcs.size_id },
      });
